...
def run(i):
    ...
    plt.ylabel("accuracy")
    plt.legend()
    plt.savefig(f"../../data/figures/gradual_unfreezing/compare_all_l_run_{i}.png")
    plt.close()
    
    test_acc = eval_test(model, dataloaders["test"], device )
    
    np.save(f"../../data/figures/gradual_unfreezing/run_{i}_train.npy", np.array(train_acc))
    np.save(f"../../data/figures/gradual_unfreezing/run_{i}_val.npy", np.array(val_acc))
    np.save(f"../../data/figures/gradual_unfreezing/run_{i}_test.npy", np.array([test_acc]))
